...
/* USER CODE BEGIN PV */
uint8_t rx_data;        // to store received byte
...
void do_action(uint8_t cmd) {
	...
	case '2':   // LED BLINK until next command
		blink_flag = 1;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	if (huart->Instance == USART1) {
		...
		/* Ignore CR/LF from terminal apps */
		if (ch == '\r' || ch == '\n') {
		...
		/* Re-enable interrupt receive for next byte */
		HAL_UART_Receive_IT(&huart1, &rx_data, 1);
	}
}
...
int main(void)
{
  ...
  char menu[] =
    ...
    /* Start UART interrupt receive on USART1 */
    HAL_UART_Receive_IT(&huart1, &rx_data, 1);
